...
const target = `<main className="max-w-md mx-auto space-y-8">`;

const replace = `<main className="max-w-md mx-auto space-y-8">
...
if (code.includes(target) && !code.includes('Player Card (Gamification)')) {
    code = code.replace(target, replace);
    fs.writeFileSync('src/pages/ClientPanel.tsx', code);
    console.log("ClientPanel updated with Player Card");
} else {
